...
function sesto_system_app_run(array $args = [], array $config = null, callable $error_handler = null): string
{
  $exit_code = 1;
  $app = [
    'config' => [],
  ...
  /* app config */
  if (null === $config) {
    $config = sesto_config(APP_CONF_DIR . '/app', false, SYS_ENV);
  }
  $app['procedure'] = $config;
  ...
  if (true) {
    ...
    $error_handler = 'sesto_system_error_handler_web';
  ...
  return sesto_app_call($func_call, $func_args, $error_handler);
}
